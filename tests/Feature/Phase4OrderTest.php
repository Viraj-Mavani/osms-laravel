...
class Phase4OrderTest extends TestCase
{
    ...
    public function test_cannot_attach_another_tenants_eye_record_to_an_order(): void
    {
        ...
        $otherPatient = Customer::create(['name' => 'Theirs', 'phone' => '222']);
        ...
        $this->actingAs($this->user);
        $customer = Customer::create(['name' => 'Ours', 'phone' => '111']);
    ...
    public function test_status_transition(): void
    {
        $this->actingAs($this->user);
        $customer = Customer::create(['name' => 'Rahul', 'phone' => '111']);
        $order = Order::create(['customer_id' => $customer->id, 'status' => 'pending', 'total_amount' => 100, 'advance_paid' => 0]);
    ...
    public function test_eye_records_json_endpoint(): void
    {
        $this->actingAs($this->user);
        $customer = Customer::create(['name' => 'Rahul', 'phone' => '111']);
        $customer->eyeRecords()->create(['tenant_id' => $this->user->tenant_id, 'od_sph' => -1]);

        $this->actingAs($this->user)->getJson(route('tenant.customers.eye-records', $customer))
